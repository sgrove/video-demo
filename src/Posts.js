...
export function PaginatedRepositoryIssues(props) {
  ...
  const post3Uses = repositoryForPaginatedIssues?.posts?.edges?.map(
    (item, idx) => <Post3 key={item?.post?.id || idx} issue={item?.post} />
  );
      ...
      query Posts_PaginatedRepositoryIssuesContainerQuery(
        ...
        oneGraphNode(oneGraphId: $oneGraphId) {
          ...
          ...Posts_repositoryForPaginatedIssues
            @arguments(count: $count, cursor: $cursor, createdBy: $createdBy)
        }
      }
    `,
  }
...
export function PostsQuery(props) {
  ...
  return (
    <div>
      {dataEl}
      <h4>
        PaginatedRepositoryIssuesUses <LocationNote />
      </h4>
