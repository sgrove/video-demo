...
export const LocationNote = () => {
  ...
  return (
    <div data-tooltip={location} ref={containerRef}
        onMouseEnter={(event) => {
          const parentNode = event.nativeEvent?.target?.parentNode;
          if (!parentNode) {
            setLocation('Unable to determine component source file');
            console.warn('No parentNode found for event: ', event);
            return;
          }
  
          const debugSource = Object.entries(parentNode).find(([key]) => {
            return (
              key.startsWith('__reactFiber') || key.startsWith('__reactInernal')
            );
          })?.[1]?._debugSource;
          ...
          if (!debugSource) {
            setLocation('Unable to determine component source file');
            console.warn(
              'No reactInternal property found on parentNode: ',
              parentNode,
              Object.entries(parentNode),
            );
            return;
          }
  
          const relativeFileName = debugSource.fileName
            .split('/')
            .slice(-2)
            .join('/');
          setLocation(
            `Edit me at ${relativeFileName}:${debugSource.lineNumber}:${debugSource.columnNumber}`,
          );
        }}>
      <svg
