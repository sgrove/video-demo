...
const COMMENT_NOTIFICATION_SUBSCRIPTION = graphql`
  subscription CommentNotification_CommentNotificationSubscription {
    github {
      issueCommentEvent(
        input: { repoOwner: "onegraph", repoName: "essay.dev" }
      ) {
        comment {
          ...Comment_fragment
        }
...
export default function CommentNotificationSubscription(props) {
  ...
  const dataEl = subscriptionResult?.data ? (
    <div className="data-box">
      <h3>
        Data for CommentNotificationSubscription <LocationNote />
      </h3>
  ...
  const actionButton = (
    ...
    >
      {needsLoginService
        ...
        : "Restart CommentNotificationSubscription"}
    </button>
  );
  ...
  return (
    <div>
      <h3>
        CommentNotificationSubscription <LocationNote />
      </h3>
      {formEl}
      {actionButton}
      {dataEl}
